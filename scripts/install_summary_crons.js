...
function buildMiddayMessage({ paths, config, summary, archives }) {
  ...
  return [
    ...
    '`ITERATION NOTES` is a persistent notes area for future directions, constraints, or ideas. Use it only as context; do not rewrite it as a task list unless the dashboard already explicitly turned a note into a task.',
...
function buildDailyCloseMessage({ paths, config, summary, archives }) {
  ...
  return [
    ...
    '`ITERATION NOTES` is a persistent notes area for future directions, constraints, or ideas. Use it only as context or remarks; do not rewrite it as pending or long-term task-list items unless the dashboard already made that conversion explicitly.',
